...
def prepare_data(df):
    ...
    #any preparation code
    #any preparation code
    run.log('Size',df.shape[0]) # it shows up under Metrics tab
...
train_ds=run.input_datasets['AutoMLE2ETraininggPipeline_Classification_dataset']
df=train_ds.to_pandas_dataframe()

df=prepare_data(df)

df.to_csv(os.path.join(args.output_path,"prepped_train_data_classification.csv"))
df.to_csv(os.path.join(args.output_path,"prepped_test_data_classification.csv"))
